...
class FNO2d(nn.Module):
    def __init__(self, modes1, modes2, width):
        ...
        # input channel is 15: input_array 11 + output_array 4 (last timestep)
...
class UFNO2d(nn.Module):
    def __init__(self, modes1, modes2, width, UNet=True):
    ...
    def forward(self, x):
        ...
        x = x.permute(0, 3, 1, 2)
        padding = [8,7]
        x = F.pad(x, [0,padding[1], 0,padding[0]]) # pad the domain if input is non-periodic
        ...
        x = x[..., :-padding[0], :-padding[1]] # pad the domain if input is non-periodic
        x = x.permute(0, 2, 3, 1)
        x = self.fc1(x)
        x = F.relu(x)
        x = self.fc2(x)
